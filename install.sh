#!/usr/bin/env bash
# created by: https://toolstud.io/bash/boilerplate.php
# template  : https://github.com/pforret/bashew/blob/master/template/normal.sh
### ==============================================================================
...
readonly script_created="2021-12-11"
...
param|1|action|action to perform: rpi
...
main() {
	#// the option $log_dir will be [$HOME/log/$script_prefix] or whatever the user specified with -l [value] or --log_dir [value]
	#// the option $tmp_dir will be [.tmp] or whatever the user specified with -t [value] or --tmp_dir [value]
	#// the parameter $action has to be specified when starting the script
  ...
  rpi)
    ...
    #TIP:> $script_prefix install input.txt
...
lookup_script_data() {
  ...
  Linux | GNU*)
    if [[ $(command -v lsb_release) ]]; then
      ...
      tmp_file=.lsb.$$.txt
      lsb_release -a &> "$tmp_file"
      os_name=$(grep 'Distributor ID:' "$tmp_file" | awk -F: '{$1=""; gsub(/^[\s\t]+/,"",$2); gsub(/[\s\t]+$/,"",$2); print $2}' ) # Ubuntu/Raspbian
      os_version=$(grep 'Release:' "$tmp_file" | awk -F: '{$1=""; gsub(/^[\s\t]+/,"",$2); gsub(/[\s\t]+$/,"",$2); print $2}' ) # 20.04
      rm "$tmp_file"
    else
      # Synology, QNAP,
      os_name="Linux"
